...
func (h *ProofHandler) Approve(w http.ResponseWriter, r *http.Request) {
	proofID, err := uuid.Parse(chi.URLParam(r, "pid"))
	if err != nil {
		http.Error(w, `{"error":"invalid proof id"}`, http.StatusBadRequest)
		return
	...
	updated, err := h.queries.ApproveProof(r.Context(), proofID)
	if err != nil {
		http.Error(w, `{"error":"failed to approve proof"}`, http.StatusInternalServerError)
		return
	}
